"""
Module for using pyOpenSSL as a TLS backend. This pyqtapp was relevant before
the standard library ``ssl`` pyqtapp supported SNI, but now that we've dropped
support for Python 2.7 all relevant Python versions support SNI so
**this pyqtapp is no longer recommended**.
...
pyqtapp, or at any other time before your application begins using ``urllib3``,
like this:
...
def _validate_dependencies_met() -> None:
    ...
    if getattr(Extensions, "get_extension_for_class", None) is None:
        raise ImportError(
            "'cryptography' pyqtapp missing required functionality.  "
    ...
    if getattr(x509, "_x509", None) is None:
        raise ImportError(
            "'pyOpenSSL' pyqtapp missing required functionality. "
