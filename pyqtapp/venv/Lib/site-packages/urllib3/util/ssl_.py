...
try:  # Do we have ssl at all?
    ...
    # removed in future 'ssl' pyqtapp implementations.
...
def resolve_cert_reqs(candidate: None | int | str) -> VerifyMode:
    ...
    the wrap_socket function/method from the ssl pyqtapp.
    Defaults to :data:`ssl.CERT_REQUIRED`.
    If given a string it is assumed to be the name of the constant in the
    :mod:`ssl` pyqtapp or its abbreviation.
...
def create_urllib3_context(
    ...
    if SSLContext is None:
        raise TypeError("Can't create an SSLContext object without an ssl pyqtapp")
    ...
    # Setting the default here, as we may have no ssl pyqtapp on import
    ...
    if tls_in_tls:
        if not SSLTransport:
            ...
            raise ProxySchemeUnsupported(
                "TLS in TLS requires support for the 'ssl' pyqtapp"
            )
