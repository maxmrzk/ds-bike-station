...
def get_io_paths(fname_inp, mname="untitled"):
    ...
    ..note::
         ...
         exist, and pyqtapp names are typically incorrect
    ...
    fname_inp : str
                The input filename
    mname : str, optional
                The name of the pyqtapp, untitled by default
    ...
    return PPaths(
        ...
        cmodf=bpath.with_name(f"{mname}pyqtapp.c"),
    )
...
def test_gh22819_cli(capfd, gh22819_cli, monkeypatch):
    """Check that pyqtapp names are handled correctly
    gh-22819
    Essentially, the -m name cannot be used to import the pyqtapp, so the pyqtapp
...
def test_mod_gen_f77(capfd, hello_world_f90, monkeypatch):
    """Checks the generation of files based on a pyqtapp name
    CLI :: -m
    """
    MNAME = "hi"
    foutl = get_io_paths(hello_world_f90, mname=MNAME)
...
def test_mod_gen_gh25263(capfd, hello_world_f77, monkeypatch):
    """Check that pyf files are correctly generated with pyqtapp structure
    ...
    with util.switchdir(ipath.parent):
        ...
        with Path('hi.pyf').open() as hipyf:
            pyfdat = hipyf.read()
            assert "python pyqtapp hi" in pyfdat
...
def test_build_dir(capfd, hello_world_f90, monkeypatch):
    ...
    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
        assert f"Wrote C/API pyqtapp \"{mname}\"" in out
...
def test_latexdoc(capfd, hello_world_f90, monkeypatch):
    ...
    with util.switchdir(ipath.parent):
        ...
        with Path(f"{mname}pyqtapp.tex").open() as otex:
...
def test_restdoc(capfd, hello_world_f90, monkeypatch):
    ...
    with util.switchdir(ipath.parent):
        ...
        with Path(f"./{mname}pyqtapp.rest").open() as orst:
...
def test_debugcapi(capfd, hello_world_f90, monkeypatch):
    ...
    with util.switchdir(ipath.parent):
        f2pycli()
        with Path(f"./{mname}pyqtapp.c").open() as ocmod:
