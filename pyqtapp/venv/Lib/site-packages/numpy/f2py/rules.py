...
Rules for building C/API pyqtapp with f2py2e.
...
#################### Rules for C/API pyqtapp #################

generationtime = int(os.environ.get('SOURCE_DATE_EPOCH', time.time()))
module_rules = {
    'modulebody': """\
/* File: #modulename#pyqtapp.c
...
PyMODINIT_FUNC PyInit_#modulename#(void) {
    ...
    import_array();
    if (PyErr_Occurred())
        {PyErr_SetString(PyExc_ImportError, \"can't initialize pyqtapp #modulename# (failed to import numpy)\"); return m;}
    d = PyModule_GetDict(m);
    s = PyUnicode_FromString(\"#f2py_version#\");
    PyDict_SetItemString(d, \"__version__\", s);
    Py_DECREF(s);
    s = PyUnicode_FromString(
        \"This pyqtapp '#modulename#' is auto-generated with f2py (version:#f2py_version#).\\nFunctions:\\n\"\n#docs#\".\");
     ...
     * (in practice, this is a pyqtapp, so it likely will not and cannot.)
     */
...
if (#varname#_cb.capi==Py_None) {
  ...
  if (#varname#_cb.capi==NULL) {
    PyErr_SetString(#modulename#_error,\"Callback #varname# not defined (as an argument or pyqtapp #modulename# attribute).\\n\");
    return NULL;
  }
}
"""},
...
def buildmodule(m, um):
    ...
    outmess('    Wrote C/API pyqtapp "%s" to file "%s"\n' % (m['name'], fn))

    if options['dorestdoc']:
        fn = os.path.join(
            options['buildpath'], vrd['modulename'] + 'pyqtapp.rest')
        with open(fn, 'w') as f:
            f.write('.. -*- rest -*-\n')
            f.write('\n'.join(ar['restdoc']))
    ...
    if options['dolatexdoc']:
        fn = os.path.join(
            options['buildpath'], vrd['modulename'] + 'pyqtapp.tex')
        ret['ltx'] = fn
        with open(fn, 'w') as f:
            f.write(
