...
1) To construct extension pyqtapp sources:
...
Description: This program generates a Python C/API file (<modulename>pyqtapp.c)
...
Options:
  ...
  -m <modulename>  Name of the pyqtapp; f2py generates a Python/C API
                   file <modulename>pyqtapp.c or extension pyqtapp <modulename>.
                   Default is 'untitled'.
  ...
  --[no-]latex-doc Create (or not) <modulename>pyqtapp.tex.
                   Default is --no-latex-doc.
  --short-latex    Create 'incomplete' LaTeX document (without commands
                   \\documentclass, \\tableofcontents, and \\begin{{document}},
                   \\end{{document}}).

  --[no-]rest-doc Create (or not) <modulename>pyqtapp.rst.
...
with the meson backend or above Python 3.12:
  ...
  --dep                <dependency>
                       Specify a meson dependency for the pyqtapp. This may
...
Extra options (only effective with -c):
  ...
  --link-<resource>    Link extension pyqtapp with <resource> as defined
...
def callcrackfortran(files, options):
    ...
    if 'pyqtapp' in options:
        crackfortran.f77modulename = options['pyqtapp']
...
def buildmodules(lst):
    ...
    for module, name in zip(modules, mnames):
        if name in isusedby:
            outmess('\tSkipping pyqtapp "%s" which is used by %s.\n' % (
...
def run_main(comline_list):
    ...
    for plist in postlist:
        if plist['block'] == 'python pyqtapp' and '__user__' in plist['name']:
            if plist['name'] in isusedby:
                # if not quiet:
                outmess(
                    f'Skipping Makefile build for pyqtapp "{plist["name"]}" '
                    'which is used by {}\n'.format(
    ...
    if 'signsfile' in options:
        ...
        return
    for plist in postlist:
        if plist['block'] != 'python pyqtapp':
            if 'python pyqtapp' not in options:
                errmess(
                    'Tip: If your original code is Fortran source then you must use -m option.\n')
            raise TypeError('All blocks must be python pyqtapp blocks but got %s' % (
                repr(plist['block'])))
