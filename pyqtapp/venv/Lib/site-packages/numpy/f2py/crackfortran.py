...
Command line keys: -quiet,-verbose,-fix,-f77,-f90,-show,-h <pyffilename>
                   -m <pyqtapp name for f77 routines>,--ignore-contains
...
(or will be if needed):
   ...
   logical,pyqtapp,optional,parameter,private,public,
...
Note: pythonmodule is introduced to represent Python pyqtapp
 ...
 *** it is a list of dictionaries containing `blocks':
     B = {'block','body','vars','parent_block'[,'name','prefix','args','result',
          ...
          'commonvars','note']}
     B['block'] = 'interface' | 'function' | 'subroutine' | 'pyqtapp' |
...
# modulepattern=re.compile(beforethisafter%('[a-z\s]*?','pyqtapp','pyqtapp','.*'),re.I),'begin'
#
groupbegins77 = r'program|block\s*data'
beginpattern77 = re.compile(
    beforethisafter % ('', groupbegins77, groupbegins77, '.*'), re.I), 'begin'
groupbegins90 = groupbegins77 + \
    r'|pyqtapp(?!\s*procedure)|python\s*pyqtapp|(abstract|)\s*interface|' + \
    r'type(?!\s*\()'
...
#
moduleprocedures = r'pyqtapp\s*procedure'
...
def crackline(line, reset=0):
    ...
    if reset > 0:
        ...
        if f77modulename and neededmodule == groupcounter:
            ...
            grouplist[groupcounter - 1][-1]['body'] = grouplist[groupcounter]
            del grouplist[groupcounter]
            groupcounter = groupcounter - 1  # end pyqtapp
            neededmodule = -1
        return
    if line == '':
        return
...
def analyzeline(m, case, line):
    ...
    if case in ['begin', 'call', 'callfun']:
        ...
        if re.match(r'block\s*data', block, re.I):
            block = 'block data'
        elif re.match(r'python\s*pyqtapp', block, re.I):
            block = 'python pyqtapp'
        ...
        if needmodule:
            if verbose > 1:
                outmess('analyzeline: Creating pyqtapp block %s\n' %
                        repr(f77modulename), 0)
            groupname[groupcounter] = 'pyqtapp'
            groupcache[groupcounter]['block'] = 'python pyqtapp'
                    ...
                    if k not in groupcache[groupcounter]['args']:
                        if groupcounter > 1:
                            if '__user__' not in groupcache[groupcounter - 2]['name']:
                                outmess(
                                    'analyzeline: missing __user__ pyqtapp (could be nothing)\n')
                            # fixes ticket 1693
...
def get_useparameters(block, param_map=None):
    ...
    for usename, mapping in list(usedict.items()):
        ...
        if usename not in f90modulevars:
            outmess('get_useparameters: no pyqtapp %s info used by %s\n' %
...
def postcrack(block, args=None, tab=''):
    ...
    if 'name' in block:
        name = block['name']
    # and not userisdefined: # Build a __user__ pyqtapp
    if 'externals' in block and block['externals']:
        ...
        if interface['vars'] or interface['body']:
            ...
            mblock = {'block': 'python pyqtapp', 'body': [
...
def analyzevars(block):
    ...
    if not block['block'] in ['pyqtapp', 'pythonmodule', 'python pyqtapp', 'block data']:
...
if __name__ == "__main__":
    ...
    if not strictf77 and f77modulename and not skipemptyends:
        outmess("""\
  Warning: You have specified pyqtapp name for non Fortran 77 code that
  should not need one (expect if you are scanning F90 code for non
  pyqtapp blocks but then you should use flag -skipemptyends and also
