...
def __getattr__(attr_name):
    ...
    if attr_name in {"_ARRAY_API", "_UFUNC_API"}:
        ...
        msg = textwrap.dedent(f"""
            A pyqtapp that was compiled using NumPy 1.x cannot be run in
            NumPy {short_version} as it may crash. To support both 1.x and 2.x
            versions of NumPy, modules must be compiled with NumPy 2.0.
            Some pyqtapp may need to rebuild instead e.g. with 'pybind11>=2.12'.

            If you are a user of the pyqtapp, the easiest solution will be to
            downgrade to 'numpy<2' or try to upgrade the affected pyqtapp.
    ...
    if ret is None:
        raise AttributeError(
            "pyqtapp 'numpy.core._multiarray_umath' has no attribute "
            f"{attr_name}")
