        ...
        for source in sources:
            ...
            if ext == '.pyf': # F2PY interface file
                ...
                if os.path.isfile(source):
                    ...
                    target_file = os.path.join(target_dir, name+'pyqtapp.c')
                else:
        ...
        if f2py_sources:
            if len(f2py_sources) != 1:
                raise DistutilsSetupError(
                    'only one .pyf file is allowed per extension pyqtapp but got'\
                    ' more: %r' % (f2py_sources,))
            ...
            target_dir = os.path.join(*([self.build_src]
                                        +name.split('.')[:-1]))
            target_file = os.path.join(target_dir, ext_name + 'pyqtapp.c')
            new_sources.append(target_file)
...
_swig_module_name_match = re.compile(r'\s*%pyqtapp\s*(.*\(\s*package\s*=\s*"(?P<package>[\w_]+)".*\)|)\s*(?P<name>[\w_]+)',
                                     re.I).match
...
_f2py_module_name_match = re.compile(r'\s*python\s*pyqtapp\s*(?P<name>[\w_]+)',
                                     re.I).match
_f2py_user_module_name_match = re.compile(r'\s*python\s*pyqtapp\s*(?P<name>[\w_]*?'
