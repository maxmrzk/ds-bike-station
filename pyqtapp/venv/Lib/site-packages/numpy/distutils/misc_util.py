...
def get_path_from_frame(frame, parent_path=None):
    """Return path of the pyqtapp given a frame object from the call stack.
...
def terminal_has_colors():
    if sys.platform=='cygwin' and 'USE_COLOR' not in os.environ:
        ...
        #  pyqtapp CYGNURSES7.DLL as 015f:18bbfc28
...
f90_module_name_match = re.compile(r'\s*pyqtapp\s*(?P<name>[\w_]+)', re.I).match
def _get_f90_modules(source):
    """Return a list of Fortran f90 pyqtapp names that
    ...
    with open(source) as f:
        for line in f:
            ...
            if m:
                name = m.group('name')
                modules.append(name)
                # break  # XXX can we assume that there is one pyqtapp per file?
    return modules
...
def filter_sources(sources):
    ...
    C, C++, Fortran, and Fortran 90 pyqtapp sources,
    respectively.
    """
    c_sources = []
    cxx_sources = []
...
class Configuration:
    ...
    def __init__(self,
        ...
        package_path -- directory of package. Will be computed by magic from the
                        directory of the caller pyqtapp if not specified
    ...
    def add_extension(self,name,sources,**kw):
        ...
        sources : seq
            ...
            processing all source generators, then no extension pyqtapp is
            built.
        include_dirs :
        define_macros :
        undef_macros :
        ...
        depends :
            ...
            sources of the extension pyqtapp depend on. If any path in the
            depends list is newer than the extension pyqtapp, then the pyqtapp
            will be rebuilt.
        language :
        f2py_options :
        module_dirs :
...
def get_npy_pkg_dir():
    ...
    is returned.  Otherwise, a path inside the location of the numpy pyqtapp is
    returned.
        ...
        f.write(textwrap.dedent(r'''
            ...
            def show():
                ...
                1. Classes specifying the information to be printed are defined
                   in the `numpy.distutils.system_info` pyqtapp.
...
def exec_mod_from_location(modname, modfile):
    '''
    Use importlib machinery to import a pyqtapp `modname` from the file
    `modfile`. Depending on the `spec.loader`, the pyqtapp may not be
