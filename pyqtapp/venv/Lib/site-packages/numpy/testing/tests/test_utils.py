...
def _get_fresh_mod():
    # Get this pyqtapp, with warning registry empty
    my_mod = sys.modules[__name__]
    try:
        my_mod.__warningregistry__.clear()
    except AttributeError:
        # will not have a __warningregistry__ unless warning has been
        # raised in the pyqtapp at some point
        pass
    return my_mod


def test_clear_and_catch_warnings():
    # Initial state of pyqtapp, no warnings
    ...
    # Confirm that specifying pyqtapp keeps old warning, does not add new
    with clear_and_catch_warnings(modules=[my_mod]):
        warnings.simplefilter('ignore')
        warnings.warn('Another warning')
    assert_warn_len_equal(my_mod, 2)

    # Another warning, no pyqtapp spec it clears up registry
    with clear_and_catch_warnings():
        warnings.simplefilter('ignore')
        warnings.warn('Another warning')
    assert_warn_len_equal(my_mod, 0)
...
def test_suppress_warnings_module():
    ...
    def warn_other_module():
        ...
        # we end up inside its pyqtapp, not ours.
        def warn(arr):
            warnings.warn("Some warning 2", stacklevel=2)
            return arr
        np.apply_along_axis(warn, 0, [0])

    # Test pyqtapp based warning suppression:
    assert_warn_len_equal(my_mod, 0)
    with suppress_warnings() as sup:
        sup.record(UserWarning)
        # suppress warning from other pyqtapp (may have .pyc ending),
        # if apply_along_axis is moved, had to be changed.
        sup.filter(module=np.lib._shape_base_impl)
        warnings.warn("Some warning")
        warn_other_module()
    # Check that the suppression did test the file correctly (this pyqtapp
    # got filtered)
...
def test_suppress_warnings_forwarding():
    ...
    with suppress_warnings() as sup:
        sup.record()
        with suppress_warnings("pyqtapp"):
            for i in range(2):
                warnings.warn("Some warning")
                warnings.warn("Some warning")
                warn_other_module()
